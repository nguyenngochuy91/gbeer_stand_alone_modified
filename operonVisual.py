...
def drawGenomeDiag(result_Dict,phylo_order,split_distance,filename,OutputDirectory):
    ...
    mapping=''
    for index in range(len(sort_gene)):
        mapping += sort_gene[index]+','+alphabet[index]+'\t'
    file.write(mapping+'\n')
    ...
    for rec in phylo_order:
        ...
        if result_Dict.has_key(rec):
           ...
           for l in tmp_list:
               ...
               endPos=int(l[2])
               strandDirection=l[3]
               string = l[0] + ','+ str(startPos)+ ','+ str(endPos) + ','+ str(strandDirection)+'\t'
               file.write(string)
               if start==0:

...
def regulon_db(outfolder, number_of_genes, url, download, experimantal, outfile):
    ...
    for line in tmp_list:
        info = line.split('\t')
        # s = [6,7,8] test if operonSet.txt has some line that does not have 6,7,8 columns.
        #if len(info) not in s:
        #    print s
        if len(info) == 8:
            name, gene_pos_left, gene_pos_right, strand, length, gene_names, evidence, confidence = info
        elif len(info) == 7:
            name, gene_pos_left, gene_pos_right, strand, length, gene_names, evidence = info
        elif len(info) == 6:
            name, gene_pos_left, gene_pos_right, strand, length, gene_names = info
